#!/usr/bin/env python3
"""
Query Production Database Script

This script allows you to run SQL queries against your production Cloud SQL database.
"""
...
def start_cloud_sql_proxy():
    ...
    try:
        ...
        if ':5432' in result.stdout:
            print("✅ Port 5432 is already in use - proxy is running")
            return True
    except:
        pass
...
def show_common_queries():
    ...
    print()
    print("7. Check user table")
    print("   SELECT COUNT(*) as total_users FROM user;")
    print()
    print("8. Check recent users")
...
def run_quick_checks(engine):
    ...
    # Check video table
    run_query(engine, "SELECT COUNT(*) as total_videos FROM video;", "TOTAL VIDEOS")
    ...
    # Check users
    run_query(engine, "SELECT COUNT(*) as total_users FROM user;", "TOTAL USERS")
...
def main():
    """Main function"""
    print("☁️ ===== PRODUCTION DATABASE QUERY TOOL =====")
    print()
    
    # Start Cloud SQL Proxy
    if not start_cloud_sql_proxy():
        print("❌ Failed to start Cloud SQL Proxy")
        print("Make sure Cloud SQL Proxy is installed and working")
        return
