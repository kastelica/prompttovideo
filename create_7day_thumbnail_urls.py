#!/usr/bin/env python3
"""
Create 7-Day Thumbnail URLs
==========================

This script creates signed URLs with 7-day expiration (maximum allowed)
for thumbnails. These URLs will need to be refreshed weekly.
"""
...
def create_7day_thumbnail_urls():
    """Create signed URLs with 7-day expiration for thumbnails"""
    ...
    try:
        ...
        # Set expiration to 7 days from now (maximum allowed)
        ...
        for video in videos:
            ...
            try:
                ...
                print(f"  ✅ Created 7-day URL (expires: {expiration.strftime('%Y-%m-%d %H:%M')})")
        ...
        if success_count > 0:
            print(f"\n✅ {success_count} thumbnails now have 7-day URLs!")
            print(f"These URLs will be valid until {expiration.strftime('%Y-%m-%d %H:%M')}")
            print("The thumbnails should now be visible on your website.")
            print()
            print("⚠️  IMPORTANT: These URLs will expire in 7 days!")
            print("You'll need to run this script again to refresh them.")
            print("Consider setting up a weekly cron job to run this script automatically.")
...
if __name__ == "__main__":
    print("CREATE 7-DAY THUMBNAIL URLS")
    print("=" * 50)
    create_7day_thumbnail_urls() 